...
create index idx_IDK_details on infra_events(infra_type, infra_ID, event_name, start_time, end_time);
create index idx_IDK_date on infra_events(infra_type, infra_ID, event_date);


/* user looking at their scheule on a given day
 * display requirements: location, event name, start and end time
 * date verification requirements: date */
create index idx eID_details on infra_events(event_ID, infra_type, infra_ID, event_name, start_time, end_time);
